			...
			$("#sel"+i+"").change(function() {
				...
				$.ajax({
					...
					success:function(data) {
						...
						var result=$("#inp").val();
						$("#nursingLevelId").val(data.rows[0].FScoreID);
						$("#updNursingLevelId").val(data.rows[0].FScoreID);
						$("#fnursingLevel").val(result);
						$("#updfnursingLevel").val(result);
					}
				});
			})
		})(i);
...
// // function queryRank(){
// // 	$.ajax({
// // 		url:contextPath+"/assess/query.do",
// // 		type:"post",
// // 		data:{
// // 			resultSum:$("#searchName").val(),
// // 			page:1,
// // 			pageSize:10
// // 		},
// // 		success:function(data) {
...
// 				$("#sel"+i+"").append("<option value='' style='display: none'>请选择一个等级</option>");
// 				for (var j=0;j<data.total;j++){
// 					$("#sel"+i+"").append("<option class='selectNum' style=' text-align:center' value='"+data.rows[j].FEvaluationValue+"'>"+data.rows[j].FLevelName+"</option>");
// 				}
// 			}
// 			$tab1.append("<tr>" +
// 				"<td style=' text-align:center; font-size: 22px;'>结果</td>" +
