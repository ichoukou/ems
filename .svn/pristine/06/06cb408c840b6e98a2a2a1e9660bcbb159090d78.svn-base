...
function open_init_comp_print(foldmanid){
...
function initPaymentTable(){
	  $('#tb_paymentPrint').bootstrapTable({
...
function reloadTotalData(){
	var rows= $('#tb_paymentPrint').bootstrapTable("getData");
	var total=getRpayTotal();
	
	var foldmanName="";
	if(rows.length>0){
		foldmanName=rows[0].foldmanName;
		$("#foldmanName").html(foldmanName+"应缴费打印");
	}
	...
	$("#paymentListPrintForm .fixed-table-footer").html(html);
