...
<body class="gray-bg">
		...
		<div class="panel-body">
			<form id="formSearch" class="form-horizontal">
				<div class="form-group" style="margin-top:15px">
					<label class="control-label col-sm-1" for="searchName">采购员</label>
					<div class="col-sm-2">
						<input type="text" class="form-control" id="searchName" placeholder="查询的名字">
					</div>

					<label class="control-label col-sm-1" for="searchEntryDate">开始时间</label>
					<div class="col-sm-2">
						<input id="searchEntryDate" class="form-control" type="text" onFocus="WdatePicker({readOnly:true,maxDate:'#F{$dp.$D(\'searchEndDate\')||\'new Date()\'}'})" style="width:120px;"/>
					</div>
					<label class="control-label col-sm-1" for="searchEntryDate">结束时间</label>
					<div class="col-sm-2">
						<input id="searchEndDate" class="form-control" type="text" onFocus="WdatePicker({readOnly:true,minDate:'#F{$dp.$D(\'searchEntryDate\')}',maxDate:new Date()})" style="width:120px;" />
					</div>
					<div class="col-sm-3" style="text-align:left;">
						<button type="button" style="margin-left:30px" id="clean_query" class="btn btn-default" onclick="cleanData();">清空</button>
						<button type="button" id="btn_query" class="btn btn-primary" onclick="queryMain();">查询</button>
					</div>
					</div>
			</form>
		</div>
	...
	<div id="toolbar" class="btn-group">
		<button id="btn_add" type="button" class="btn btn-primary" data-toggle="modal" data-target="#addModal" onclick="gotoAdd()">
			<span class="glyphicon glyphicon-plus" aria-hidden="true"></span>新增采购
		</button>
		...
		<button id="btn_delete" type="button" class="btn btn-default" onclick="gotoDelete()">
			<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>删除采购单
		</button>
	</div>

	<table id="tb_resEnterWarehouse"></table>
</div>
 
	<!-- 控制修改按钮 -->
	<button id="update_btn" type="button" class="btn btn-default" data-toggle="modal" data-target="#updateModal" style="display:none">
	   </button>
	<button id="addGoods_btn" type="button" class="btn btn-default" data-toggle="modal" data-target="#addGoodsModal" style="display:none">
	</button>
	<jsp:include  page="./resEnterWareHouseAdd.jsp"/>
	<jsp:include  page="./wareHouseUpdate.jsp"/>
	<jsp:include  page="./resEnterWareHouseUpdate.jsp"/>
	...
	<script src="${pageContext.request.contextPath}/script/app-resources/storage-goods/resEnterWarehouse.js"></script>
