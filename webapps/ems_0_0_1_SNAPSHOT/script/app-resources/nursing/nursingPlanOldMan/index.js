...
function chooseOldMan(){
	$.ajax({
        type: "POST",
        url: contextPath+"/nursingPlanOldMan/chooseOldMan.do",
        success: function(data){
       	 	open_chooseOldMan_page(data);
    	 }
	});
}

//打開选择老人頁面窗口
function open_chooseOldMan_page(html){
	var title="选择老人";
	$.confirm({
        title: '<h2>'+title+'<h2>',
        content : html,
...
function reloadDataMergeCells(){
	...
	//执行完过后调用该方法
	$("#tb_nursingPlanOldMan").on("load-success.bs.table", function(){
		if(flag){
			var datas = $("#tb_nursingPlanOldMan").bootstrapTable('getData');
			var merges = [];
			...
			for(var i = 0; i < datas.length; i++){
				...
				$.ajax({
					type : 'POST',
					url :contextPath+"/nursingPlanOldMan/queryDetailList.do",
					...
					success : function(details) {
						if(details.success){
							$("#tb_nursingPlanOldMan").bootstrapTable('removeByUniqueId', details.rows[0].fServiceGroupID);  //删除本行
							var merge = {};
							...
							for(var j=0;j<details.rows.length;j++){
								...
								$("#tb_nursingPlanOldMan").bootstrapTable('insertRow', {
									index: position++,
									row: row
								});
							}
					...
					complete : function(){
						if(finishCount == totalOrderCount){
							for(var k=0; k < merges.length; k++) {
								var m = merges[k];
								$("#tb_nursingPlanOldMan").bootstrapTable('mergeCells', {
									index: m.index,
...
//保存老人护理项目
function saveOldManNursingPlan(){
	var foldmanid=$("#searchOldManFid").val();
		if(foldmanid){
		var rows=$("#tb_nursingPlanOldMan").bootstrapTable("getSelections");
		if(rows.length <1){
			alertx("请选择一条数据!");
		}else{
			var fids="";
			...
			$.ajax({
				type : 'POST',
				url : contextPath+"/nursingPlanOldMan/saveOldManServiceItem.do",
				async: false,
				data : {fids : fids,foldmanid : foldmanid},
				success : function(rs) {
					if(rs.success){
						alertx(rs.msg);
						//保存完了之后重新刷新表数据
						queryNursingPlanOldMan();
					}else{
						alertx(rs.msg);
					}
				},
...
function initOldManServiceItemTb(){
	  $('#tb_nursingPlanOldMan').bootstrapTable({
