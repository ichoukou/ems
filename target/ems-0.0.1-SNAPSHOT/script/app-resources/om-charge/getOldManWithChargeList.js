...
$(document).ready(
    ...
    // 列出查询的房间输入框
    getAllRoom_chargeList(),
    // 列出查询的老人状态
    getAllOldManState_chargeList()
);


function getAllRoom_chargeList(){
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#searchFroom_chargeList").html(room);
        }
    });
}

function getAllOldManState_chargeList(){
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#searchFoldManStatusID_chargeList").html(oldManState);
        }
    });
}


function initChooseOldManTb_charge(){
    $('#tb_chooseOldMan_chargeList').bootstrapTable({
        ...
        queryParams: function (params) {
            var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
                ...
                foldManName : $("#searchFOldManName_chargeList").val(), //老人姓名
                froomID : $("#searchFroom_chargeList").val(), //房间
                // fOldManNumber : $("#searchFOldManNumber").val(), //老人编号
                foldManStatusID : $("#searchFoldManStatusID_chargeList").val() //老人状态
            };
            return temp;
        },//传递参数（*）
        ...
        onLoadSuccess : function (data){
            ...
            var showHtml=getShowHtml_chargeList(data);
            //$("#tb_chooseOldMan").html("");
            //$("#chooseOldMan .fixed-table-body").html(showHtml);
            $("#tb_chooseOldMan_chargeList").html(showHtml);
        }
    });
};

function getShowHtml_chargeList(data){
    var html="<tr><td>";
    ...
    if(dataLength>0){
        ...
        for(var i=0;i<rowLength;i++){
            ...
            for(var j=start;j<end;j++){
                ...
                var dataHtml=getOneTdHtml_chargeList(oldMan);
                html+=dataHtml;
                html+="</div>";
            }
...
function getOneTdHtml_chargeList(obj){
    var fPhotoSrc=contextPath+"/picture/"+obj.fPhoto;
    var html="<table class='table table-bordered'>";
    html+="<tr>";
    html+="<td rowspan='4'><img style='height: 135px;width: 110px;' name='' src='"+fPhotoSrc+"' onclick=\"getOldManWithPhotoWithCharge('"+obj.fID+"','"+obj.foldManName+"')\" /></td>";
...
function queryOldMan_chargeList(){
    $('#tb_chooseOldMan_chargeList').bootstrapTable('refresh');
}

//清空老人查询条件
function resetSearchOldMan_chargeList(){
    $("#searchFOldManName_chargeList").val("");//老人姓名
    $("#searchFroom_chargeList").val(""); //房间
    $("#searchFoldManStatusID_chargeList").val(""); //老人状态
}
